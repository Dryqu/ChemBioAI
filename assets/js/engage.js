...
const CONFIG = {
  // CLERK KEYS (publishable key is safe to expose in frontend)
...
async function initAuth() {
  // If no key, still load comments + likes (likes will be local-only)
  ...
  if (!window.Clerk) {
    ...
    script.src =
      "https://accounts.clerk.dev/npm/@clerk/clerk-js@latest/dist/clerk.browser.js";
    script.async = true;
  ...
  try {
    if (!pk || !pk.startsWith("pk_") || pk.includes("YOUR_KEY_HERE")) {
      console.warn(
        "Engage Module: Clerk Publishable Key missing/placeholder. Auth disabled."
      );
      clerkReady = false;
      clerk = null;
      user = null;
    ...
    console.error(
      "Engage Module: Failed to initialize Clerk (script blocked / invalid key / network).",
      e
    );
    clerkReady = false;
    clerk = null;
  ...
  // Enable like button (works even without Clerk; Clerk just adds identity)
  enableLikeButton();
}
...
function enableLikeButton() {
  ...
  likesData = JSON.parse(localStorage.getItem("articleLikes") || "{}");
  const currentLikes = likesData[articleId] || { count: 0, users: [] };

  likeCount.textContent = currentLikes.count;

  // Mark liked if signed-in user already liked
  if (user && currentLikes.users.includes(user.id)) {
    likeBtn.classList.add("liked");
  }

  likeBtn.addEventListener("click", async () => {
    // If Clerk is configured but still loading, tell user to retry
    const pk = CONFIG.clerkPublishableKey;
    const clerkConfigured =
      pk && pk.startsWith("pk_") && !pk.includes("YOUR_KEY_HERE");

    if (clerkConfigured && !clerkReady) {
      alert("Authentication is still loading. Please try again in a moment.");
      return;
    }

    // If Clerk is configured and ready, require sign-in for persistent identity
    if (clerkConfigured) {
      if (!user) {
        try {
          await clerk.openSignIn();
          // After sign-in, Clerk should populate user
          user = clerk.user;

          // If still no user (user closed modal), stop
          if (!user) return;
        } catch (e) {
    ...
    if (isLiked) {
      ...
      currentLikes.count++;
      if (user) {
        // Only track user ids when signed in
        if (!currentLikes.users.includes(user.id)) currentLikes.users.push(user.id);
      }
      likeBtn.classList.add("liked");
    }
...
function loadComments() {
  ...
  if (!CONFIG.giscusRepo || CONFIG.giscusRepo === "YourUsername/ChemBioAI") {
    container.innerHTML =
      '<p style="text-align:center; padding: 2rem; color: #64748b; border: 1px dashed #cbd5e1; border-radius: 0.5rem;">Comments are disabled until Giscus is configured.</p>';
    return;
  }
